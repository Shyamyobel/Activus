...
const SMEValidateDocument = () => {
  ...
  const fetchTDSData = async () => {
    ...
    try {
      const response = await axios.get(
        'http://localhost:8080/api/tds/pendingSMEValidation',
        {
      ...
      setError(err.response?.data?.message || 'Failed to fetch documents for validation');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleValidation = async (tdsId, isApproved) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8080/api/tds/validateDocumentBySME/${tdsId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { 
      ...
      setError(err.response?.data?.message || 'Failed to process validation');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>SME Document Validation</h2>
  ...
  {tds.documentPath ? (
    ...
    <span>No Documents</span>
                  ...
                  <td style={styles.td}>
                    <button
                      onClick={() => handleValidation(tds.tdsId, true)}
                    ...
                    <button
                      onClick={() => handleValidation(tds.tdsId, false)}
              ...
              <tr>
                <td colSpan="4" style={styles.noTDS}>
                  No documents found for validation.
                </td>
              </tr>
            )}
          </tbody>
...
export default SMEValidateDocument; 