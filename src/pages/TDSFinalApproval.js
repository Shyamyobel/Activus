...
const TDSFinalApproval = () => {
  ...
  const fetchTDSData = async () => {
    ...
    try {
      const response = await axios.get(
        'https://activus-server-production.up.railway.app/api/tds/need-to-approve/l2',
        {
      ...
      setError(err.response?.data?.message || 'Failed to fetch TDS for final approval');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApproval = async (tdsId, isApproved) => {
    ...
    try {
      const response = await axios.put(
        `https://activus-server-production.up.railway.app/api/tds/approve/l2/${tdsId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { 
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>L2 Approval</h2>
    ...
    <span>No Documents</span>
              ...
              <tr>
                <td colSpan="7" style={styles.noTDS}>
                  No TDS items found for final approval.
                </td>
              </tr>
            )}
          </tbody>
...
export default TDSFinalApproval;