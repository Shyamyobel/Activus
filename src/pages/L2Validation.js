...
const L2Validation = () => {
  ...
  const fetchTDSData = async () => {
    ...
    try {
      const response = await axios.get(
        'https://activus-server-production.up.railway.app/api/tds/need-to-approve/l2-after-l3',
        {
      ...
      setError(err.response?.data?.message || 'Failed to fetch TDS for validation');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApproval = async (tdsId) => {
    setIsLoading(true);
    setError('');
    setSuccess('');
    ...
    try {
      const response = await axios.put(
        `https://activus-server-production.up.railway.app/api/tds/approve/l2-after-l3/${tdsId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
          params: { username }
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>TDS Final Approval</h2>
  ...
  {tds.documentPath ? (
    ...
    <span>No Documents</span>
                  ...
                  <td style={styles.td}>
                    <button
                      onClick={() => handleApproval(tds.tdsId)}
              ...
              <tr>
                <td colSpan="7" style={styles.noTDS}>
                  No TDS items found for validation.
                </td>
              </tr>
            )}
          </tbody>
